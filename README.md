...
- 가사 싱크 정확도: 100ms 폴링 + 타겟 스케줄링(다음 가사 직전 정확 폴링) + 200ms 선행 보상
- 앱 실행 시 이미 재생 중인 곡도 정확한 위치에서 가사 동기화 (SMTC `LastUpdatedTime` 보정)
- 이전곡/다음곡 버튼 누를 때 즉시 타임라인 리셋
- 가사 백그라운드 로딩 — 곡 전환 시 UI 블로킹 없음
...
- 가사 로컬 캐시 (최대 500곡, 스레드 안전, 반복 API 호출 없음)
...
| **HTTP**          | requests                     | 2.34.2+ | LRCLIB / iTunes API 호출            |
...
Python ──spawn──▶ PowerShell (pwsh 우선, 없으면 PS5.1 폴백)
                    ...
                    └─ 250ms 루프: 현재 재생 정보 → JSON → stdout
                         position = Get-AdjustedPosition(tl)
                                  = tl.Position + (UtcNow - tl.LastUpdatedTime)
                                    │
Python ◀──readline──┘
...
- `Get-AdjustedPosition`: SMTC `LastUpdatedTime` 기반으로 실제 재생 위치 복원 (Melon 등 position=0 고정 앱 대응)
- 앨범아트: SMTC 썸네일(임시 파일 경유) → iTunes Search API 폴백

### 2. 가사 조회 (lyrics_engine.py)

곡 변경 감지 시 백그라운드 스레드에서 비동기로 실행 → UI 블로킹 없음.

```
get_lyrics(title, artist)   ← 백그라운드 스레드에서 호출
    │
    ├─ 1) CacheManager.get()         → 캐시 히트 시 즉시 반환
    ├─ 2) LRCLIB /api/get (4s)       → 200: LRC 파싱 / 404: /api/search 폴백
    ├─ 3) LRCLIB /api/search (8s)    → 폴백 검색 (느릴 수 있어 별도 타임아웃)
    ├─ 4) 아티스트 없이 재시도
    └─ 5) 결과 없음 → 빈 배열 캐시  (재요청 방지)
```

- LRC 형식: `[mm:ss.xx] 가사 텍스트` → `(timestamp_seconds, text)` 리스트
- plain 가사(타임스탬프 없음): 3초 간격 균등 배치
- 캐시 키: `SHA-256(title::artist)[:16]`
- CacheManager: `threading.Lock` 보호 (백그라운드 스레드 동시 접근 안전)

### 3. 재생 위치 실시간 추정 (api.py)

SMTC position은 250ms마다 업데이트되므로, 그 사이 갭을 monotonic 시계로 보완합니다.

```python
estimated = pos_base + (now - pos_base_time) + 0.20  # 200ms lookahead
```

**Seek/이전곡 감지**: SMTC position 자체의 변화량(`abs(new - prev) > 3.0s`)으로 판단.
Melon처럼 position=0 고정 앱은 `media_prev()` / `media_next()` 호출 시 즉시 리셋.

**타겟 스케줄링**: 응답에 `ms_to_next` 포함 → JS가 다음 가사 직전 정확한 시점에 폴링.
...
| JS 호출                      | Python 메서드                  | 동작                                      |
| ---------------------------- | ------------------------------ | ----------------------------------------- |
| `get_current_state()`        | `LyricApi.get_current_state()` | 현재 가사·재생 상태 + ms_to_next 반환     |
...
[Windows SMTC에 현재 재생 정보 등록]
            ...
            ▼ (250ms마다)
[PowerShell SMTC 폴러]
  → Get-AdjustedPosition() = Position + (UtcNow - LastUpdatedTime)
  → title, artist, position(보정됨), duration, is_playing
            │
            ▼
[MediaSessionPoller._parse_line()]
  → MediaInfo 객체에 저장 (thread-safe lock)
            │
            ▼ (JS에서 100ms 또는 ms_to_next마다 get_current_state() 호출)
[LyricApi.get_current_state()]
  ┌─ 곡 변경 감지 (song_id 비교)?
  │     YES → 백그라운드 스레드(lyrics-fetch) 시작
  │              ├─ 캐시 히트 → 즉시 반환
  │              └─ LRCLIB /api/get(4s) → 404면 /api/search(8s) → 캐시 저장
  │           UI: "가사 불러오는 중..." → 로드 완료 시 자동 전환
  └─ 재생 위치 추정(200ms lookahead) → get_current_index() → get_context()
     + ms_to_next(다음 가사까지 ms) 계산
                   → {이전가사, 현재가사, 다음가사, ms_to_next}
            │
            ▼
[JS updateLyrics()] → 화면 가사 업데이트 (100ms fade 애니메이션)
[JS tick 타겟 스케줄링] → setTimeout(tick, min(ms_to_next, 100))
...
`.venv/`는 `.gitignore`에 포함되어 있습니다.

### 배포용 exe 빌드

```bash
# venv 활성화 후
pyinstaller showlyric.spec --clean --noconfirm
